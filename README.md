...
This automates deploying AWS's IPSEC VPN solution and configuring the BIG-IPs. It also serves as a reference implementation.
...
* Two F5 BIG-IPs
  ...
  * Per AWS documentation, the addresses used for the customer gateways (BIG-IP Self IPs) can not sit behind a NAT (which precludes testing with VEs in AWS).
* Software version == BIG-IP v12.0.0+
  * Versions Tested so far: 
  	* BIG-IP v12.0.0 Build 0.0.606
* In order to run above playbooks
  * Have your public key installed in /root/.ssh/authorized_keys
...
2. Change Passwords (on each device)

	```
	modify auth password root <will prompt> 
	modify auth user admin password <password>
	```
	NOTE: 
	commands are assuming you're in tmsh. If starting in bash, enter tmsh by typing "tmsh"
	ex.
	```
...
3. License (customize the "hostname" per device)
	```
...
4. Configure Global Settings (on each device)
	```
...
5. Configure Network: (customize the "unique" self-ips on each device). 
	```
	Vlans:
	create net vlan internal interfaces add { 1.1 }
	create net vlan external interfaces add { 1.2 }

	Unique Self-IPs: (device unique and will NOT sync)
...
7. Configure IPSEC Global Settings (on each device)
...
8. Configure IKE Peers (on bigip-01 as will sync)
...
9. Configure IPSEC Policy (on bigip-01 as will sync)
...
10. Configure IPSEC Selectors (on bigip-01 as will sync)
...
11. Configure IPSEC Profiles (on bigip-01 as will sync).  

	```
	create net tunnels ipsec aws_conn_1_tun_1_profile app-service none defaults-from ipsec traffic-selector /Common/aws_conn_1_tun_1_selector 

	create net tunnels ipsec aws_conn_1_tun_2_profile app-service none defaults-from ipsec traffic-selector /Common/aws_conn_1_tun_2_selector 

	create net tunnels ipsec aws_conn_2_tun_1_profile app-service none defaults-from ipsec traffic-selector /Common/aws_conn_2_tun_1_selector 

	create net tunnels ipsec aws_conn_2_tun_2_profile app-service none defaults-from ipsec traffic-selector /Common/aws_conn_2_tun_2_selector 
	```

12. Configure Tunnel Keep Alive Pools (on bigip-01 as will sync).
...
15. Configure BGP (customize per each device as will NOT sync). ex. 
	...
	Create a file named ZebOS.conf with variables for each device that looks like example_configs/cache/bigip-01_ZebOS.conf:
	```
	!
	...
	Copy it to /config/zebos/rd0/ZebOS.conf ( on each device )

	Then run (on each device)
	```
	modify net route-domain 0 routing-protocol add { BGP }
	```


17. OPTIONAL if you want to failover based on number of tunnels available, you can create an HA group. 
...
Although some of the objects will be unique to a BIG-IP or AWS Connection, they will be shared in the cluster.
For a complete example configs of the clustered pair, see the example-configs directory.

* Full SCF backups of devices used for testing (to show items that have been shared )
	* bigip-01.scf
	* bigip-02.scf
* VPN Specific Config Objects for each device
...
2. On your automation host:
  	...
  	* edit "~/.aws/credentials" to provide credentials for boto. ex.
		...
		```
 	* edit default location of your private key in ansible_cfg (bigip-aws-vpn/ansible_cfg)

		```private_key_file=~/.ssh/<your-key-name>.pem```

  	* edit "~/creds.yml"  - a custom credential file for this repo. ex.
...
3. Run playbooks from within bigip-aws-vpn directory. Set the env_tag for name of your deployment.
	...
	```ansible-playbook playbooks/delete-vpn.yml --extra-vars '{"env_tag":"example_vpn"}'```
  	...
  	* Configures on-prem customer gateway BIG-IPs
	```ansible-playbook playbooks/configure-bigips.yml --extra-vars '{"env_tag":"example_vpn"}'```
	```TODO: reset config = tmsh load /sys config default```
